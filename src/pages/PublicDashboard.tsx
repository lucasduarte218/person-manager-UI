...
import { Search, Edit, Trash2, Plus, Users, Calendar, Mail, MapPin, LogIn } from 'lucide-react';
...
export default function PublicDashboard() {
  ...
  const fetchPersons = async () => {
    try {
      setIsLoading(true);
      const data = await apiService.getPersonsV1();
      setPersons(data);
  ...
  const handleEdit = (id: number) => {
    navigate(`/public/person/edit/${id}`);
  };

  const handleNewPerson = () => {
    navigate('/public/person/new');
  };
